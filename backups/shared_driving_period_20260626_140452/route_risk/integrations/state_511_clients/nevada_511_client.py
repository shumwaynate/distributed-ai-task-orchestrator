...
def apply_nevada_recurrence_timing(
    ...
    """Reclassify events using recurring daily active windows."""
    ...
    window = (
        _nevada_description_time_window(
            updated_event
        )
        or _nevada_schedule_time_window(
            updated_event
        )
    )
    ...
    days_of_week = (
        window.get("days_of_week")
        or _nevada_schedule_days(
            updated_event
        )
    )

    nevada_timezone = ZoneInfo(
        "America/Los_Angeles"
    )
    ...
    local_reference = reference_utc.astimezone(
        nevada_timezone
    )
    ...
    active_now = _nevada_window_is_active(
        local_datetime=local_reference,
        start_time=start_time,
        end_time=end_time,
        days_of_week=days_of_week,
    )

    next_start_local = _next_nevada_window_start(
        local_datetime=local_reference,
        start_time=start_time,
        days_of_week=days_of_week,
    )
    ...
    updated_event.update(
        {
            "recurrence_active_now": active_now,
            "recurrence_window_start_local": (
                start_time.strftime("%H:%M:%S")
            ),
            "recurrence_window_end_local": (
                end_time.strftime("%H:%M:%S")
            ),
            "recurrence_timezone": (
                "America/Los_Angeles"
            ),
            "recurrence_window_source": (
                window["source"]
            ),
            "recurrence_days_of_week": sorted(
                days_of_week
            ),
            "next_occurrence_iso_local": (
                next_start_local.isoformat()
                if next_start_local
                else None
            ),
            "next_occurrence_iso_utc": (
                next_start_utc.isoformat()
                if next_start_utc
                else None
            ),
            "next_occurrence_in_hours": (
                round(
                    next_start_hours,
                    2,
                )
                if next_start_hours is not None
                else None
            ),
        }
    )
    ...
    if (
        next_start_hours is not None
        and next_start_hours
...
def fetch_nevada_511_event_groups(
    ...
    for raw_event in raw_events:
        ...
        normalized_event = apply_nevada_recurrence_timing(
